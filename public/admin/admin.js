...
(function () {
  ...
  var pusher = new Pusher("c6af91b1dcd27455b15d", {
  ...
  const helpers = {
    ...
    LoadParticipant: (evt) => {
      ...
      if (chat.currentParticipant !== undefined) {
        ...
        chatBody
          .find("#dismissParticipant")
          .off("click")
          .on("click", helpers.TerminateUserConnection);
        chatBody
          .find("#warnParticipant")
          .off("click")
          .on("click", helpers.SendWarning);
      }
      evt.preventDefault();
    },
  ...
  chatChannel.bind("pusher:member_removed", (member) => {
    var remainingUsers = chat.subscribedUsers.filter(
